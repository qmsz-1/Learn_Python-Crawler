...
obj = re.compile(r'<li>.*?<div class="item">.*?<span class="title">(?P<name>.*?)</span>.*?'
                 r'<div class="bd">.*?<p>(?P<director>.*?)&nbsp;(?P<actor>.*?)'
                 r'<br>(?P<year>.*?)&nbsp;/&nbsp;(?P<country>.*?)&nbsp;/&nbsp;(?P<type>.*?)</p>', re.S)
...
for a in range(0, 250, 25):
    ...
    for it in result:
        print(it.group("name"), it.group("director").strip(), "\n", it.group("actor").strip("..."))
        print(it.group("year").strip(), it.group("country"), it.group("type"))
    resp.close()